    ...
    <div className="bg-f1-surface border border-f1-border rounded-sm p-4">
      ...
      <div className="flex flex-col gap-3 mb-4">
        {stints.map((stint, index) => {
          const compoundColor = tireColors[stint.compound]
          return (
            <div
              key={index}
              className="bg-f1-bg border border-f1-border rounded-sm p-3"
              style={{ borderLeftWidth: 3, borderLeftColor: compoundColor }}
            >
              {/* Stint header row */}
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <TireCompoundIcon compound={stint.compound} size="md" />
                ...
                {availableCompounds.map((compound) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={isLocked}
                      className={`
                        flex items-center gap-1 font-pixel text-[9px] px-2.5 py-1.5 border rounded-sm transition-colors cursor-pointer
                        disabled:opacity-30 disabled:cursor-not-allowed
                        ${
                          isActive
                            ? 'border-current bg-f1-surface'
                            : 'border-f1-border hover:border-f1-border bg-f1-bg'
                        }
                      `}
                      style={{
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  className="w-16 bg-f1-bg border border-f1-border rounded-sm px-2 py-1 font-pixel text-[10px] text-f1-text text-center outline-none focus:border-f1-accent transition-colors"
                />
      ...
      <button
        ...
        className="w-full border-2 border-dashed border-f1-border hover:border-f1-accent/60 rounded-sm py-2 font-pixel text-[9px] text-f1-text/50 hover:text-f1-accent transition-colors mb-4 cursor-pointer"
      >
        + ADD PIT STOP
      </button>
