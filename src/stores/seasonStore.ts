...
export const useSeasonStore = create<SeasonState>((set, get) => ({
  ...
  purchaseBranchUpgrade: (area, branch) => {
    ...
    const branchNode = branch === 'a'
      ? rdTree[area].branches[0]
      : rdTree[area].branches[1]
  ...
  addRaceResults: (results) => {
    ...
    const updatedDriverStandings = state.driverStandings.map((ds) => {
      const driverResult = results.driverPositions.find(
        (dp) => dp.driverId === ds.driverId,
      )
      const driverPoints = results.pointsPerDriver.find(
        (pp) => pp.driverId === ds.driverId,
      )
    ...
    const updatedTeamStandings = state.teamStandings.map((ts) => {
      const teamDriverIds = drivers
        .filter((d) => d.teamId === ts.teamId)
        .map((d) => d.id)
